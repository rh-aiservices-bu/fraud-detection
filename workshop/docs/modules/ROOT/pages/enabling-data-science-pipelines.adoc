...
NOTE: If you do not intend to complete the pipelines section of this {deliverable} you can advance to xref:setting-up-kueue-resources.adoc[Setting up Kueue resources].
...
. Locate the *Access key* field next to the key icon, and click the dropdown menu
. Click *Pipeline Artifacts* to populate the *Configure pipeline server* with credentials for the connection.
...
[IMPORTANT]
====
You must wait until the pipeline configuration is complete before you create your workbench. 
====
+
If more than 5 minutes have elapsed and the pipeline server configuration is not complete, delete it and recreate it.
+
