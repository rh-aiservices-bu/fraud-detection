...
You can run a script to install local object storage buckets and create connections. The script automatically completes the following tasks:
...
* Installs the required network policies for service mesh functionality.

The script is based on a https://ai-on-openshift.io/tools-and-applications/minio/minio/[guide for deploying MinIO].

IMPORTANT: The MinIO-based Object Storage that the script creates is **_NOT_** meant for use in a production environment.

.Prerequisites

* You have the resource name for your data science project. To get the project's resource name:

** In the {productname-short} dashboard, select *Data science projects* and then click the *?* icon next to the project name. A text box appears with the resource name.
//image::projects/ds-project-list-resource-hover.png[Project list resource name, 400]

[NOTE]
====
The following procedure describes how to run the script from the OpenShift console. Alternatively, you can access the cluster from the command line interface (CLI) using the following command:
...
+
NOTE: This code retrieves and applies the `setup-s3-no-sa.yaml` file.
+
[.lines_space]
[.console-input]
[source, yaml]
...
[IMPORTANT]
====
If your cluster uses self-signed certificates, your {productname-short} administrator might need to provide a certificate authority (CA) to securely connect to the S3 object storage.
