...
In previous sections of this {deliverable}, you used a notebook to manually train and save your model. Pipelines can automate this process for multiple notebooks and Python code. 

In this section, you create a simple pipeline by using the GUI pipeline editor. The pipeline uses your previously created notebook to train a model and save it in S3 storage. Your completed pipeline should look like the one in the `6 Train Save.pipeline` file.

NOTE:
You can skip the following procedure and instead run the `6 Train Save.pipeline` file.
...
You must set the S3 storage bucket keys by using the secret created by the `My Storage` connection that you set up in the storing-data-with-connections[Storing data with connections] section of this {deliverable}.
