...
. Click *OK*.
+
NOTE: If `Data Science Pipeline` is not available as a runtime configuration, you may have created your notebook before the pipeline server was available. Also verify that you did the prerequisite step of xref:enabling-data-science-pipelines.adoc[setting up a pipeline server].  You can restart your notebook after the pipeline server has been created in your data science project. 
