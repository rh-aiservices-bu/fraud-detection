:_module-type: PROCEDURE

[id='deploying-the-model']
= Deploying the model

[role="_abstract"]

Given that the model is accessible in storage and saved in the portable ONNX format, you can use an {productname-short} model server to deploy it as an API.

.Prerequisites

* You saved the model as described in xref:preparing-the-model-for-deployment.adoc[Preparing the model for deployment].

* You have installed KServe and enabled the model serving platform.

* You have enabled a preinstalled or custom model-serving runtime.

* You have values for the following MinIO storage:
** Access Key
** Secret Key
** Endpoint
** Region
** Bucket
+
To obtain these values, navigate to your project's *Connections* tab. For the MyStorage connection, click the action menu (&#8942;) and then click *Edit*.

.Procedure

. In the {productname-short} dashboard, navigate to the project details page and click the *Deployments* tab.
+
image::model-serving/ds-project-model-list-add.png[Deployments]
+
NOTE: Depending on the model serving configuration on your cluster, you might see only one model serving platform option.

. In the *Enable model serving* tile, click *Select single-model*.
. Click *Deploy model*.
+
The *Deploy a model* wizard opens.
. In the *Model details* section, provide information about the model:
.. For the *Model location*, select *S3 compatible object storage*.
.. Enter the values for the following for your MinIO storage conenction:
** Access Key
** Secret Key
** Endpoint
** Region
** Bucket
.. For *Path*, enter `models/fraud`.
.. For the *Name*, enter `MyStorage`.
.. For *Model type*, select  *Predictive*.
.. Click *Next*.
. In the *Model deployment* section, configure the deployment:
.. For *Model deployment name*, enter `fraud`.
.. In the *Description* field, enter a description of your deployment.
.. For the hardware profile, accept the default.
.. For *Model framework (name - version)*, select `onnx-1`.
.. For the *Serving runtime* field, accept the auto-selected runtime, `OpenVINO Model Server`.
.. Click *Next*.
. In the *Advanced settings* section, accept the defaults by clicking *Next*.
. In the *Review* section, click *Deploy model*.

.Verification

* Confirm that the deployed model is shown on the *Deployments* tab for the project, and on the *Deployments* page of the dashboard with a *Started* status.

image::model-serving/ds-project-model-list-status.png[Deployed model status, 500]

.Next step

xref:testing-the-model-api.adoc[Testing the model API]
