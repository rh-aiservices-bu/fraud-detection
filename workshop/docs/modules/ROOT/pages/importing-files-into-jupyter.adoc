...
[role="_abstract"]
The JupyterLab environment is web-based, but all operations run on {productname-long} and are backed by the OpenShift cluster. This configuration allows you to run notebooks without installing local tooling or consuming local CPU, GPU, or memory resources.
...
. Verify that the Git version at the bottom of the JupyterLab window is *{git-version}*.
