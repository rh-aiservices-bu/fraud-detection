...
For convenience, run a provided script that completes the following tasks for you:
...
* Creates two Data Connections in your data science project, one for each bucket and both using the same credentials.
* Installs required network policies for service mesh functionality.

This script is based on a https://ai-on-openshift.io/tools-and-applications/minio/minio/[guide for deploying Minio].

IMPORTANT: The Minio-based Object Storage that this script creates is *not* meant for production usage.
